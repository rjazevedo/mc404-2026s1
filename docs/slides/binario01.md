...
- Exemplos de caracteres com mais de um byte em UTF-8:
  - ¢ (centavo): Unicode U+00A2 → UTF-8 C2 A2 (2 bytes)
  - € (euro): Unicode U+20AC → UTF-8 E2 82 AC (3 bytes)
  - 😀 (grinning face): Unicode U+1F600 → UTF-8 F0 9F 98 80 (4 bytes)

- Caracteres “parecidos” (homoglyphs) para segurança — visualmente quase idênticos
  - Latino a (U+0061) vs Cirílico а (U+0430) 
  - Latino o (U+006F) vs Grego ο (U+03BF)
  - Latino p (U+0070) vs Cirílico р (U+0440)
  - Latino A (U+0041) vs Grego Α (U+0391)

## Representação UTF-8

- Considerando a representação U+uvwxyz, onde u, v, w, x, y e z são as 6 letras hexadecimais do código Unicode, a codificação UTF-8 é definida da seguinte forma (note que cada letra representa 4 bits):

| de       | até      | Byte 1   | Byte 2   | Byte 3   | Byte 4   |
| -------- | -------- | -------- | -------- | -------- | -------- |
...
* Cada caracter está em uma ou mais posições de memória consecutivas
...
* Cada byte em memória tem um endereço individual
